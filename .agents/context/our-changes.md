...
13. **CosyVoice2 codec frame rate is 25fps (40ms/frame, 24kHz output)** — 25 new frames = 1s audio after left-context trimming. Without trimming: 2s per chunk. Common pitfall: assuming output size without measuring. To diagnose: if total audio = 2× expected, trimming is broken; if = expected, trimming works.
14. **left_context_size async_chunk data flow** — Request.additional_information (plain dict) → OmniCachedRequestData.additional_information (via OmniGenerationScheduler line 118+246) → _update_additional_information() (in _preprocess, guarded by `enable_update_additional_information=True`) → model_intermediate_buffer → runtime_additional_information → forward(). All stages confirmed correct.
