...
| 영어 음성 대화 (6 시나리오) | 단어 정확도 92%, 지연 2.3s | A |
| 중국어 음성 대화 (2 시나리오) | CER 76.1% / 의미 95% | B+ |
| 한국어 음성 대화 | TTS 완전 실패 | F (TTS only) |
| **VoiceBench 전체** (텍스트 전용) | 평균 점수 98.0% / pass rate 98.7% | A+ |
...
| 메트릭 | 값 | 비고 |
|--------|---|------|
| STT 정확도 (단어 정확도) | **92%** | Whisper base, 6시나리오 평균 |
...
| 클라이언트 `modality=="audio"` 감지 | 수정 완료: `getattr` 기본값 `None`, modality 먼저 검사, audio 청크 누적 처리 | ✅ 수정 완료 |
